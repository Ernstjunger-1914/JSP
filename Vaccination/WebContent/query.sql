...
select pname, j.jumin, decode(substr(j.jumin, 8, 1), '1', '남', '2', '여') as gender, tel, substr(resvdate, 1, '4')||'년'||substr(resvdate, 5, 2)||'월'||substr(resvdate, 7, 2)||'일' as resvdate, substr(resvtime, 1, 2)||':'||substr(resvtime, 3, 2) as resvtime, decode(h.hospcode, 'H001', '가-병원', 'H002', '나-병원', 'H003', '다-병원', 'H004', '라-병원') as hosp, hosptel, hospaddr, decode(vcode, 'V001', 'A백신', 'V002', 'B백신', 'V003', 'C백신') as vname from tbl_vaccresv_201808 v inner join tbl_hosp_201808 h on v.hospcode=h.hospcode inner join tbl_jumin_201808 j on j.jumin=v.jumin and resvno='20210001';
